...
function useT() {
  ...
  // zh-hant needs its own lookup so traditional Chinese doesn't fall through to simplified
...
function CityForgeToolbarButton() {
  ...
  return <FloatingButton src={cityForgeIcon} selected={panelVisible} onClick={()=>trigger("cheatmod","TogglePanel")}/>;
}
...
function CityForgePanel() {
  ...
  return (
    <div onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp}
      style={{position:"fixed" as const,left:`${pos.x}px`,top:`${pos.y}px`,width:`${size.w}px`,height:`${size.h}px`,minWidth:`${MIN_W}px`,maxWidth:`${MAX_W}px`,minHeight:`${MIN_H}px`,background:C.bg,border:`1px solid ${C.border}`,borderRadius:"10rem",boxShadow:"0 10rem 40rem rgba(0,0,0,0.75)",fontFamily:"'Segoe UI','Noto Sans',Arial,sans-serif",fontSize:"14rem",color:C.textPrimary,userSelect:"none" as const,zIndex:9999,overflow:"hidden",display:"flex",flexDirection:"column" as const,pointerEvents:"auto" as const}}>
      ...
      <div style={{flex:"1 1 0",padding:`${pad}rem`,overflowY:"auto" as const,pointerEvents:"auto" as const}}>
        ...
        <div style={{textAlign:"center" as const,fontSize:"11rem",color:C.textMuted,padding:"2rem 0"}}>{t(KEYS.dragHint)}</div>
      </div>
